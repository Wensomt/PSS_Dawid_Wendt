...
app = FastAPI(
    ...
    description="CRUD notatek + prosty X-API-Key + paginacja (page/size), q, sort asc/desc po created_at.",
    version="0.1.0",
)
...
# >>> CHANGED: API_KEY z .env, fallback 'secret'
...
def list_notes(q: str | None = None, sort: str = "desc"):
    db = load_db()
    data = db["notes"]
    if q:
        ql = q.lower()
        data = [n for n in data if ql in n["title"].lower()
